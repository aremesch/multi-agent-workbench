...
export const DELETE: RequestHandler = async ({ locals, params, url, cookies, request }) => {
  ...
  // No-worktree spawn (adapter createWorktree=false): the worktrees row
  // points at the repo root, so there's nothing to remove from disk and
  // running the dirty-check against the repo root would just surface the
  // user's own unrelated work.
  const sharesRepoRoot = !!(worktree && repo && worktree.path === repo.path);
  ...
  if (removeWorktree && worktree && repo) {
    if (sharesRepoRoot) {
      // Nothing to `git worktree remove` — just mark the row.
