...
async function call(opts: CallOpts = {}): Promise<Response> {
  ...
  const request = new Request('http://localhost/api/user/push-preferences', {
    method: 'PUT',
...
describe('PUT /api/user/push-preferences', () => {
  it('403 when CSRF verification fails', async () => {
    await expectHttpError(
      call({ csrfThrows: true, body: { kinds: ['prompt_detected'] } }),
      403
    );
  });

  it('401 when not signed in', async () => {
    await expectHttpError(call({ user: null, body: { kinds: [] } }), 401);
  });

  it('400 when body is not valid JSON', async () => {
    await expectHttpError(call({ rawBody: 'oops' }), 400);
  });

  it('400 when kinds is not an array', async () => {
    await expectHttpError(call({ body: { kinds: 'all' } }), 400);
  });

  it('400 when kinds contains an unknown string', async () => {
    await expectHttpError(call({ body: { kinds: ['typo_kind'] } }), 400);
  });

  it('204 and persists an empty array', async () => {
    const res = await call({ body: { kinds: [] } });
  ...
  it('204 and persists every supported kind', async () => {
    const kinds = ['prompt_detected', 'task_done', 'error', 'exited'];
    const res = await call({ body: { kinds } });
    expect(res.status).toBe(204);
    expect(setUserSettingMock).toHaveBeenCalledWith(
      'user-1',
