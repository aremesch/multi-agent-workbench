...
async function call(opts: CallOpts = {}): Promise<Response> {
  ...
  const request = new Request('http://localhost/api/user/mobile-quickkeys-state', {
    method: 'PUT',
...
describe('PUT /api/user/mobile-quickkeys-state', () => {
  it('403 when CSRF verification fails', async () => {
    await expectHttpError(call({ csrfThrows: true, body: { mode: 'auto' } }), 403);
  });

  it('401 when not signed in', async () => {
    await expectHttpError(call({ user: null, body: { mode: 'auto' } }), 401);
  });
  ...
  it('400 when mode is unknown', async () => {
    await expectHttpError(call({ body: { mode: 'sometimes' } }), 400);
  });

  it.each(['auto', 'always', 'never'])(
    '204 and persists for mode=%s',
    async (mode) => {
      const res = await call({ body: { mode } });
