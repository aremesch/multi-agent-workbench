...
export function insertAgent(row: {
  ...
  const ts = now();
  prep<
    [string, string, string, string, string, string, string, AgentStatus, number, number]
  >(
    `INSERT INTO agents
       (id, user_id, role_id, repo_id, worktree_id, cli_kind, tmux_session, status, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
  ).run(
    row.id,
    row.user_id,
