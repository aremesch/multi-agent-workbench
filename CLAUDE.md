...
- [`docs/plans/v0.2-fix-xterm-minify-undeclared-i.md`](docs/plans/v0.2-fix-xterm-minify-undeclared-i.md) — fixes the `Show Log` modal crash on TUI agents (claude-code/codex/gemini) where Firefox printed `Uncaught ReferenceError: assignment to undeclared variable i` from `BqvuqXEL.js` (the bundled xterm.js chunk) and painted the modal blank. Root cause was Vite's default esbuild minifier mishandling the ES2021 `let r; ...(r ||= {})` pattern xterm 6.0.0 uses for the function-scoped `ModeReportType` enum inside `InputHandler.requestMode`: esbuild dropped the `let r;` declaration but kept the renamed `i = {}` assignment, producing `(void 0||(i={}))` against an undeclared `i` that strict-mode ESM rejects on every incoming DECRQM byte sequence. Fix is a one-config-line minifier swap to **terser** in [`vite.config.ts`](../../vite.config.ts) (`build: { minify: 'terser' }`) plus the new `terser` dev-dep — terser handles `||=` correctly. Verified by re-grepping the rebuilt chunk: `requestMode(e,t){let i;var s;(s=i||(i={}))[s.NOT_RECOGNIZED=0]=...` (binding present). Build time +1 s, no source/test changes (executed).
