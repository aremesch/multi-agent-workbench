...
- [`docs/plans/v0.2-tmux-survive-restart.md`](docs/plans/v0.2-tmux-survive-restart.md) — dedicated `tmux -L maw` socket started in its own `systemd-run --user --scope` unit so agents survive `systemctl --user restart maw`; drops the stale-channel guard so `session-closed` hook reliably closes the terminal modal on CLI exit (executed).
